...
class BaseUbl(models.AbstractModel):
    ...
    def _ubl_add_address(self, partner, node_name, parent_node, ns, version="2.1"):
        ...
        # if partner.additional_number:
        #     additional_number = etree.SubElement(address, ns["cbc"] + "AdditionalNumber")
        #     additional_number.text = partner.additional_number
        if partner.plot_id:
            plot_id = etree.SubElement(address, ns["cbc"] + "PlotIdentification")
            plot_id.text = partner.plot_id
        if partner.city:
            citysubdivi = etree.SubElement(address, ns["cbc"] + "CitySubdivisionName")
            citysubdivi.text = partner.sub_division_name or partner.city
        if partner.city:
    ...
    def _ubl_add_delivery(self, delivery_partner, parent_node, ns, version="2.1"):
        ...
        if self.move_type not in  ('in_invoice','in_refund'):
            ...
            ################################Added############################
            # self.ubl_parse_delivery_details(delivery, ns)
            latestdeli_date = etree.SubElement(delivery, ns["cbc"] + "LatestDeliveryDate")
            latestdeli_date.text = str(self.invoice_date_due)
    ...
    def ubl_parse_address(self, address_node, ns):
        ...
        additional_xpath = address_node.xpath("cbc: Additional number", namespaces=ns)
        ...
        address_dict = {
            ...
            or False,
            "additional_number": additional_xpath and additional_xpath[0].text or False,
...
class ResPartner(models.Model):
    ...
    sub_division_name = fields.Char(string="Sub Division")
